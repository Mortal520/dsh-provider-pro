...
async function sendProbeRequest(
  ...
  const answer = await new Promise<HostProbeResult | undefined>((resolve) => {
    ...
    // pair, four levels) plus the image stream — measured first tokens on
    // the reasoning relay alone reach ~40s, and the worst-case wire sum
    // (baseline retry 2×10s + role pair 2×10s + parallel levels 2×10s)
    // rides on top. 90s keeps headroom without hanging the UI.
    timer = setTimeout(() => done(undefined), 90000)
