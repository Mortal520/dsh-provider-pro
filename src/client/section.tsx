...
interface HostProbeResult {
  ...
  maxTokens?: number
  backfilled?: number
  /** Per-stage wall-clock timings (ms), published so the UI can show where
   * the probe time went — evidence the verdict is a real measurement. */
  stages?: { baselineMs?: number; devMs?: number; sysMs?: number; streamMs?: number }
  error?: string
}
...
async function sendProbeRequest(
  ...
  if (!answer.ok) {
    ...
    const compactWireError = (raw: string): string => {
  ...
  if (answer.mode === 'full') {
    ...
    if (stages !== undefined) {
      ...
      if (stages.baselineMs !== undefined) stageParts.push(`base ${stages.baselineMs}ms`)
      if (stages.devMs !== undefined) stageParts.push(`dev ${stages.devMs}ms`)
      if (stages.sysMs !== undefined) stageParts.push(`sys ${stages.sysMs}ms`)
      if (stages.streamMs !== undefined) stageParts.push(`stream ${stages.streamMs}ms`)
