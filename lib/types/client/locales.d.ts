...
export declare const zh: {
    ...
    readonly probe: '探测';
    readonly probeHint: '一次实测：上下文窗口（网关声明，缺失则回填）+ developer 角色兼容（被拒自动改用 system）+ 图片输入（真实流式请求，自动同步声明）。';
...
export declare const en: {
    ...
    readonly probeHint: 'One pass measures: context window (gateway-declared, backfilled when missing) + developer-role compat (auto-falls back to system when refused) + image input (real streaming request, declaration synced).';
