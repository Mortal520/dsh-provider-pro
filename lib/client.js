		...
		*/
		async function sendProbeRequest(api, provider, model, events, mode) {
			...
			if (!answer.ok) {
				...
				const infraMatch = raw.match(/(?:lookup\s+([a-z0-9.-]+)|dial\s+tcp[^"]*?([a-z0-9.-]+(?:\.\w+)+)|no such host|network is unreachable)/i);
				if (infraMatch !== void 0) {
					const host = infraMatch?.[1] ?? infraMatch?.[2];
					const detail = host !== void 0 ? `（无法解析/连接 ${host}）` : infraMatch?.[0] !== void 0 ? `（传输层错误: ${infraMatch[0]}）` : "";
					return {
						status: "failure",
						provider,
						model,
						...
						failure: {
							code: "INFRA",
							message: `网关基础设施故障${detail} — DNS/网络恢复后重试`
						}
					};
				}
