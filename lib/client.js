...
`;
		...
		function ProviderProSection(props) {
			...
			* whose dictionary is byte-for-byte the auto-filled one. */
			const flipAuto = async (next) => {
				if (view === void 0 || flipBusy) return;
				setFlipBusy(true);
		...
		const zh = {
			...
			autoReasoningHint: "开启（默认）：所有自定义模型在对话的模型选择器里出现完整推理等级，初始档位为「Default」（请求不带思考参数、由供应方决定），随时可在聊天里切换。关闭：移除自动补全的档位回到供应方默认；你自己手动设过的等级不受影响。",
		...
		const en = {
			...
			autoReasoningHint: "On (default): every custom model shows the full reasoning-level set in the chat model picker, starting at \"Default\" (no thinking parameter sent; the provider decides) — switch any time in chat. Off: auto-filled levels are stripped back to provider defaults; levels you set manually are untouched.",
