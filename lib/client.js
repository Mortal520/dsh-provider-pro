		...
		async function probeModel(llmWire, provider, model) {
			...
			try {
				...
				if (typeof discoverFn !== "function") {
					const fnKeys = Object.keys(llmWire).filter((k) => typeof llmWire[k] === "function");
					return {
						status: "failure",
						provider,
						model,
						...
						failure: {
							code: "NO_DISCOVER",
							message: `No discovery method. Fns: [${fnKeys.join(", ")}]`
						}
					};
				}
				const raw = await discoverFn("llm-pi-ai", { provider });
				const found = (Array.isArray(raw) ? raw : raw.value ?? []).find((m) => m.id === model);
				...
				if (!found) return {
					...
					failure: {
						code: "NOT_FOUND",
						message: `Model "${model}" not discoverable via provider "${provider}"`
					}
				};
				const caps = [];
			...
			const runProbe = async () => {
				if (!llmWire) {
					setProbeResult({
						...
						failure: {
							code: "NO_LLM",
							message: "LLM wire not available"
						}
					});
					return;
				}
