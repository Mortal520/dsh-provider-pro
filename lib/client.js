		...
		*/
		async function sendProbeRequest(api, provider, model) {
			...
			while (Date.now() < deadline) {
				...
				if (slot !== void 0 && typeof slot === "object") {
					const result = slot;
					if (result.id === id) {
						if (!result.ok) return {
							status: "failure",
						...
						const caps = [];
						if (result.contextWindow !== void 0) caps.push(`ctx: ${result.contextWindow}`);
						if (result.maxTokens !== void 0) caps.push(`max: ${result.maxTokens}`);
						caps.push(`first-token: ${result.firstTokenMs ?? "n/a"}ms`);
						if (result.totalMs !== void 0) caps.push(`total: ${result.totalMs}ms`);
						caps.push(`reason: ${result.finishReason ?? "stop"}`);
						if (result.imageProbe) caps.push("image: accepted");
						if (result.backfilled !== void 0 && result.backfilled > 0) caps.push(`backfilled: ${result.backfilled}`);
						return {
							status: "success",
							provider,
							model,
