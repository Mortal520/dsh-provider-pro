...
`;
			...
			try {
				...
				if (typeof listModelsFn !== "function") {
					...
					return {
						...
						failure: {
							code: "NO_LIST_MODELS",
							message: `llm.listModels not a function. Available: [${methods.join(", ")}]`
						}
					};
				}
				const found = (await listModelsFn(provider)).find((m) => m.id === model);
				...
				if (!found) return {
					...
					failure: {
						code: "NOT_FOUND",
						message: `Model "${model}" not in provider "${provider}" catalog`
					}
				};
				const caps = [];
		...
		*/
		...
		function apply(ctx) {
			...
			let llmWire;
			try {
				const candidate = c.remote?.llm;
				if (candidate !== void 0 && candidate !== null && typeof candidate === "object") llmWire = candidate;
			} catch {}
			const injected = () => ({
