		...
		async function probeModel(llm, provider, model) {
			...
			try {
				...
				for await (const chunk of stream) {
					if (chunk.type === "text" && firstTokenMs === null) firstTokenMs = Math.round(performance.now() - startedAt);
					if (chunk.type === "usage" && chunk.usage?.completionTokens !== void 0) completionTokens = chunk.usage.completionTokens;
					if (chunk.type === "finish") finishReason = chunk.reason ?? "stop";
				}
			...
			const runProbe = async () => {
				if (!llm) {
					setProbeResult({
						...
						failure: {
							code: "NO_LLM",
							message: "LLM runtime not available"
						}
					});
					return;
				}
