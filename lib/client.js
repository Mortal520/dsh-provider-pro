		...
		*/
			...
			if (!answer.ok) {
				...
				if (infraMatch !== void 0) {
					...
					return {
						...
						failure: {
							code: "INFRA",
							message: `网关基础设施故障${host !== void 0 ? `（无法解析/连接 ${host}）` : ""} — 模型与插件均无问题；网关上游 DNS/网络恢复后重试`
						}
					};
				}
		...
		function ProviderCard(props) {
			...
			const providerAlive = (() => {
				...
				if (values.every((r) => r.status === "failure" && (r.failure?.cooldownUntil === void 0 || r.failure.cooldownUntil > Date.now())) && values.some((r) => r.failure?.cooldownUntil !== void 0)) return "cooldown";
				return "down";
			})();
