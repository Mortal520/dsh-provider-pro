	...
	factory: (require) => {
		...
		*/
		async function mutateFresh(api, ops, attempts = 4) {
			...
			return {
				ok: false,
				error: { message: "settings kept changing while writing (retried 4x) — try again" }
			};
		}
		...
		*/
		async function sendProbeRequest(api, provider, model, events, mode) {
			...
			const answer = await new Promise((resolve) => {
				...
				timer = setTimeout(() => done(void 0), 11e4);
				readSlot().then((value) => {
			...
			if (answer === void 0) {
				(async () => {
					const ns = (await api.describe().then((r) => r.ok ? r.value : void 0).catch(() => void 0))?.namespaces.find((entry) => entry.ns === NS);
					const req = (ns?.user ?? ns?.value ?? {})[PROBE_REQ_FLAG];
					if (req === void 0 || typeof req !== "object" || req.id !== id) return;
					await mutateFresh(api, [{
						op: "unset",
						path: [PROBE_REQ_FLAG]
					}]).catch(() => void 0);
				})();
			...
			if (!answer.ok) {
				...
				const resetSeconds = resetSecondsRaw !== void 0 ? Number(resetSecondsRaw) : resetTimeHms !== void 0 ? resetTimeHms.split(/[hms]/).filter(Boolean).reduce((acc, part, i) => acc + Number(part) * [
					3600,
					60,
					1
				].at(i), 0) : void 0;
				const cooldownUntil = resetSeconds !== void 0 && Number.isFinite(resetSeconds) ? Date.now() + resetSeconds * 1e3 : void 0;
				const isTransport = /^unreachable:/i.test(raw.trim());
				const netMatch = raw.match(/(?:getaddrinfo\s+enotfound|econnrefused|econnreset|etimedout|epipe|ehostunreach|enetunreach|lookup\s+([a-z0-9.-]+)|dial\s+tcp[^"]*?([a-z0-9.-]+(?:\.\w+)+)|no such host|network is unreachable)/i);
				if (netMatch !== null) {
					const host = netMatch[1] ?? netMatch[2];
					const detail = host !== void 0 ? `（无法解析/连接 ${host}）` : `（网络层错误: ${netMatch[0]}）`;
					return {
						status: "failure",
						provider,
						model,
						...
						failure: isTransport ? {
							code: "INFRA",
							message: `网关不可达${detail} — 网关主机 DNS/网络故障，恢复后重试`
						} : {
							code: "UPSTREAM",
							message: `网关正常，该模型上游线路故障${detail} — 模型侧问题，稍后重试此模型`
						}
					};
				}
				const code = cooldownUntil !== void 0 ? "COOLDOWN" : answer.imageSupported === false ? "IMAGE_UNSUPPORTED" : "PROBE_FAIL";
			...
			if (answer.mode === "full") {
				...
				if (stages !== void 0) {
					const stageParts = [];
					if (stages.baselineMs !== void 0) stageParts.push(`base ${stages.baselineMs}ms`);
					if (stages.devMs !== void 0) stageParts.push(`dev ${stages.devMs}ms`);
					if (stages.sysMs !== void 0) stageParts.push(`sys ${stages.sysMs}ms`);
					if (stages.streamMs !== void 0) stageParts.push(`stream ${stages.streamMs}ms`);
					if (stageParts.length > 0) parts.push(stageParts.join("/"));
				}
				return {
					status: "success",
					provider,
		...
		function ModelRow(props) {
			...
			const displayResult = (() => {
				if (probeResult === void 0) return probeAllResult;
				if (probeAllResult === void 0) return probeResult;
			...
			const runProbe = async () => {
				...
				try {
					const result = {
						...await sendProbeRequest(api, route, model.id, events, "full"),
						receivedAt: Date.now()
					};
			...
			const aliveStatus = displayResult === void 0 ? "unknown" : displayResult.status === "success" ? "up" : displayResult.failure?.cooldownUntil !== void 0 && displayResult.failure.cooldownUntil > Date.now() ? "cooldown" : "down";
			const cooldownHint = aliveStatus === "cooldown" && displayResult?.status === "failure" ? (() => {
				const mins = Math.max(1, Math.round((displayResult.failure.cooldownUntil - Date.now()) / 6e4));
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						title: cooldownHint ?? aliveStatus
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
		...
		function ProviderCard(props) {
			...
			const probeAll = async () => {
				...
				try {
					...
					for (const model of profile.models) {
						...
						if (priorCooldown !== void 0 && priorCooldown > now) {
			...
			const providerAlive = (() => {
				...
				for (const model of profile.models ?? []) {
					...
					if (hit !== void 0) live.set(model.id, hit);
				}
					...
					Array.isArray(profile.models) && profile.models.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: profile.models.map((model, idx) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ModelRow, {
								...
								onProbeResult: (modelId, result) => setProbeAllResults((prev) => {
		...
		const zh = {
			...
			probing: "探测中…",
			probe: "探测",
			probeHint: "一次实测：上下文窗口（网关声明，缺失则回填）+ developer 角色兼容（被拒自动改用 system）+ 图片输入（真实流式请求，自动同步声明）。",
			probeAll: "一键探测"
		};
		const en = {
			nav: "Model Enhance",
			...
			probing: "Probing…",
			probe: "Probe",
			probeHint: "One pass measures: context window (gateway-declared, backfilled when missing) + developer-role compat (auto-falls back to system when refused) + image input (real streaming request, declaration synced).",
			probeAll: "Probe all"
		};
		//#endregion
