			...
			const runProbe = async () => {
				...
				if (!probeApi) {
					setProbeResult({
						...
						failure: {
							code: "NOT_INSTALLED",
							message: "dsh-provider-probe not installed"
						}
					});
					return;
				}
		...
		*/
		...
		function apply(ctx) {
			...
			*/
			const resolveProbe = () => {
				try {
					const svc = ctx.get("remote:providerProbe");
					if (svc !== void 0 && svc !== null) {
						const ns = svc;
						if (typeof ns.catalog === "function" && typeof ns.probe === "function") return ns;
					}
				} catch {}
				try {
					const probeNs = c.remote?.providerProbe;
					if (typeof probeNs?.catalog === "function" && typeof probeNs?.probe === "function") return probeNs;
				} catch {}
			};
			const injected = () => ({
