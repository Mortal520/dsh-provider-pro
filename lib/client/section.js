...
function ModelRow(props) {
    ...
    return (_jsxs("div", { className: "dpp-model-row", children: [_jsx("span", { className: "dpp-model-id", title: model.id, children: model.id }), _jsxs("label", { className: "dpp-model-cb", children: [_jsx("input", { type: "checkbox", checked: hasImage, onChange: (e) => void toggleImage(e.target.checked) }), _jsx("span", { className: "dpp-model-cb-label", children: t('imageInput') })] }), probeApi ? (_jsx("button", { type: "button", className: "dpp-probe-btn", disabled: probeBusy, onClick: () => void runProbe(), children: probeBusy ? t('probing') : t('probe') })) : null, probeResult ? (_jsx("span", { className: 'dpp-probe-result ' + (probeResult.status === 'success' ? 'dpp-probe-ok' : 'dpp-probe-fail'), children: probeResult.status === 'success'
                    ? `${probeResult.firstTokenMs ?? '—'}ms / ${probeResult.totalMs ?? '—'}ms / ${probeResult.finishReason ?? '—'}`
                    : `${probeResult.failure?.code ?? 'ERR'}: ${probeResult.failure?.message ?? ''}` })) : null] }));
}
/* --------------------------------------------------------- provider (UA) card */
function ProviderCard(props) {
    ...
    return (_jsxs("div", { className: "dpp-card", role: "group", "aria-label": displayName, children: [_jsxs("div", { className: "dpp-card-header", children: [_jsx("span", { className: "dpp-provider", children: displayName }), _jsx("code", { className: "dpp-code", children: route }), _jsx("span", { className: "dpp-muted", children: baseURLMeta })] }), _jsxs("div", { className: "dpp-field", children: [_jsx("label", { className: "dpp-label", htmlFor: `ua-${route}`, children: t('userAgent') }), _jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }, children: [_jsx("input", { id: `ua-${route}`, className: "dpp-input", type: "text", value: ua, placeholder: t('userAgentPlaceholder'), disabled: busy, onChange: (event) => {
                                    ...
                                    setUa('');
                                    setDirty(true);
                                }, children: t('resetUa') })) : null] }), _jsx("span", { className: "dpp-muted", children: t('userAgentHint') }), _jsxs("div", { className: "dpp-foot", children: [_jsx("button", { type: "button", className: "dpp-primary", disabled: busy || !dirty, onClick: () => void save(), children: busy ? t('saving') : t('save') }), saved ? (_jsx("span", { className: "dpp-ok", role: "status", children: t('saved') })) : null, failure !== undefined ? (_jsx("span", { className: "dpp-error", role: "status", children: failure })) : null] })] }), Array.isArray(profile.models) && profile.models.length > 0 ? (_jsxs("div", { className: "dpp-models", children: [_jsx("span", { className: "dpp-models-title", children: t('models') }), profile.models.map((model, idx) => (_jsx(ModelRow, { route: route, model: model, modelIndex: idx, revision: revision, api: api, t: t, probeApi: probeApi, onMutated: onSaved }, model.id)))] })) : null] }));
}
/* ---------------------------------------------------------------- root UI */
