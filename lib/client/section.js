...
async function sendProbeRequest(api, provider, model, events, mode) {
    ...
    if (answer.totalMs !== undefined)
        caps.push(`total: ${answer.totalMs}ms`);
    caps.push(`reason: ${answer.finishReason ?? 'stop'}`);
    if (answer.imageProbe)
        caps.push('image: accepted');
    if (answer.backfilled !== undefined && answer.backfilled > 0)
        caps.push(`backfilled: ${answer.backfilled}`);
    return {
        status: 'success', provider, model, totalMs: answer.totalMs,
...
function ModelRow(props) {
    ...
    return (_jsxs("div", { className: "dpp-model-row", children: [_jsx("span", { className: 'dpp-alive-dot ' + aliveStatus, title: aliveStatus }), _jsx("span", { className: "dpp-model-id", title: model.id, children: model.id }), declaredCaps.length > 0 ? (_jsx("span", { className: "dpp-model-caps", title: capsTitle, children: declaredCaps.map((n, i) => _jsx("span", { className: "dpp-caps-chip", children: fmtTokens(n) }, i)) })) : null, _jsxs("label", { className: "dpp-model-cb", children: [_jsx("input", { type: "checkbox", checked: hasImage, onChange: (e) => void toggleImage(e.target.checked) }), _jsx("span", { className: "dpp-model-cb-label", children: t('imageInput') })] }), _jsx("button", { type: "button", className: "dpp-probe-btn", disabled: probeBusy, onClick: () => void runProbe('capabilities'), children: probeBusy ? t('probing') : t('capabilities') }), _jsx("button", { type: "button", className: "dpp-probe-btn", disabled: probeBusy, onClick: () => void runProbe('deep'), children: probeBusy ? t('probing') : t('deepProbe') }), displayResult ? (_jsx("span", { className: 'dpp-probe-result ' + (displayResult.status === 'success' ? 'dpp-probe-ok' : 'dpp-probe-fail'), children: displayResult.status === 'success'
