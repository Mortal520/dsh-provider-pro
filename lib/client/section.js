...
async function mutateFresh(api, ops, attempts = 4) {
    for (let i = 0; i < attempts; i++) {
        ...
        if (ns === undefined)
            return { ok: false, error: { message: 'llm-pi-ai namespace not readable' } };
        ...
        if (!/changed since it was read|expected revision/i.test(message))
...
async function sendProbeRequest(api, provider, model, events, mode) {
    ...
    if (answer === undefined) {
        ...
        void (async () => {
            ...
            const ns = view?.namespaces.find((entry) => entry.ns === NS);
            const user = (ns?.user ?? ns?.value ?? {});
            const req = user[PROBE_REQ_FLAG];
            if (req === undefined || typeof req !== 'object' || req.id !== id)
                return;
    ...
    if (!answer.ok) {
        ...
        // Two distinct failure domains that previously collapsed into one INFRA
        // code (and flooded the UI with misleading "网关基础设施故障"):
        // 1. INFRA — the plugin's OWN transport to the gateway failed at the
        //    network level (DNS, TCP, TLS). Host wire errors are prefixed
        //    "unreachable:". The gateway host itself is unreachable → retry
        //    after DNS/network recovers.
        // 2. UPSTREAM — the gateway ANSWERED (HTTP body / stream error text)
        //    and that body carries the gateway's own upstream error
        //    (new-api-class relays embed Go-style "dial tcp: lookup
        //    api.x.com: no such host" from THEIR upstream providers). The
        //    gateway is alive; only this model's upstream route is broken —
        //    model-side, not gateway infra, not a plugin bug.
        ...
        if (netMatch !== null) {
            ...
            // Hostless branches (ENOTFOUND / network unreachable / …) have no
    ...
    if (answer.mode === 'full') {
        ...
        else if (answer.imageVerdict === 'unsupported')
            parts.push('image: unsupported');
        ...
        // Stage timings turn the verdict into visible evidence: the user sees
        // exactly which phase consumed the wall-clock time.
        const stages = answer.stages;
        if (stages !== undefined) {
            const stageParts = [];
            if (stages.baselineMs !== undefined)
                stageParts.push(`base ${stages.baselineMs}ms`);
            if (stages.devMs !== undefined)
                stageParts.push(`dev ${stages.devMs}ms`);
            if (stages.sysMs !== undefined)
                stageParts.push(`sys ${stages.sysMs}ms`);
            if (stages.streamMs !== undefined)
                stageParts.push(`stream ${stages.streamMs}ms`);
...
function ModelRow(props) {
    ...
    const runProbe = async () => {
        ...
        try {
            ...
            const err = error;
            const result = { status: 'failure', provider: route, model: model.id, totalMs: 0, receivedAt: Date.now(), failure: { code: 'ERROR', message: err.message ?? String(error) } };
            setProbeResult(result);
            onProbeResult?.(model.id, result);
        }
        finally {
