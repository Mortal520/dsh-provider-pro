 ...
 * `PROBE_RESULT_FLAG`.
 */
async function sendProbeRequest(api, provider, model) {
        ...
        if (slot !== undefined && typeof slot === 'object') {
            ...
            if (result.id === id) {
                ...
                if (result.contextWindow !== undefined)
                    caps.push(`ctx: ${result.contextWindow}`);
                if (result.maxTokens !== undefined)
                    caps.push(`max: ${result.maxTokens}`);
                caps.push(`first-token: ${result.firstTokenMs ?? 'n/a'}ms`);
                if (result.totalMs !== undefined)
                    caps.push(`total: ${result.totalMs}ms`);
                caps.push(`reason: ${result.finishReason ?? 'stop'}`);
                if (result.imageProbe)
                    caps.push('image: accepted');
                if (result.backfilled !== undefined && result.backfilled > 0)
                    caps.push(`backfilled: ${result.backfilled}`);
                return {
