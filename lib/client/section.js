...
async function probeModel(llmWire, provider, model) {
    const startedAt = performance.now();
    try {
        // discoverModels(settingsNs, { provider }) — DSH typert remote API
        const discoverFn = llmWire.discoverModels;
        ...
        const raw = await discoverFn('llm-pi-ai', { provider });
        const models = Array.isArray(raw) ? raw : (raw.value ?? []);
        const found = models.find((m) => m.id === model);
        const totalMs = Math.round(performance.now() - startedAt);
        if (!found) {
            const discovered = models.map((m) => m.id).join(', ');
        ...
        if (found.name && found.name !== found.id)
            caps.push(`name: ${found.name}`);
        if (found.contextWindow)
            caps.push(`ctx: ${found.contextWindow}`);
        if (found.maxTokens)
            caps.push(`max: ${found.maxTokens}`);
        return {
    ...
    const probeAll = async () => {
        ...
        for (const model of profile.models) {
            ...
            if (Array.isArray(model.input))
                caps.push(`input: [${model.input.join(', ')}]`);
            if (model.reasoningEfforts)
                caps.push(`reasoning: ${Object.keys(model.reasoningEfforts).join('/')}`);
