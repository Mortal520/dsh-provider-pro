...
async function sendProbeRequest(api, provider, model, events, mode) {
    ...
    if (!answer.ok) {
        ...
        // Narrow scope: only match errors that are unambiguously gateway
        // infrastructure (DNS, network, TLS cert) — NOT model-level timeouts
        // or aborts which happen when the upstream is simply unreachable or
        // in cooldown. Generic "timeout"/"aborted" are model-level, not infra.
