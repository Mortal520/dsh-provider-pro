...
async function probeModel(llmWire, provider, model) {
    ...
    try {
        ...
        const discoverModelsFn = (llmWire.discoverModels ?? llmWire.listModels);
        ...
        const caps = [];
        if (found.inputModalities?.includes('image'))
            caps.push('image');
        if (found.inputModalities?.includes('text'))
            caps.push('text');
        if (found.reasoning)
            caps.push('reasoning');
        if (found.maxTokens)
            caps.push(`maxTokens:${found.maxTokens}`);
        return {
